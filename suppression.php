...
//suppression des enregistrements pour les admins
session_start();
...
if((isset($_POST['suppr']) && ($_POST['suppr'] != ''))){$Supprimer = true ;} else {$Supprimer = false ;}
...
if($Supprimer){
	...
	$sql = "DELETE FROM `gd_dns` WHERE `nom`='$ref_suppr';";

	$result = mysql_query($sql);
	if (!$result){die('Requete invalide : ' . mysql_error());}
	else{
			$Resultat = "Suppression de l'enregistrement $ref_suppr effectuée avec succès.";
			...
			$db_selected = mysql_select_db($nombdd, $link);
			if (!$db_selected) {die ('Impossible de selectionner la base de donnees : ' . mysql_error());}
						$sql2 = "INSERT INTO `gd_journal` (`utilisateur`, `action`, `ip`, `nom`, `date`, `HHmmss`) VALUES ('".mysql_real_escape_string($nomusr)."', 'Suppression de l\'enregistrement $ref_suppr effectuée avec succès.', '".mysql_real_escape_string($ip_visiteur)."', '', '".mysql_real_escape_string($date_jour)."', '".mysql_real_escape_string($heure_connexion)."');";
...
<html>
	...
	<body>
		<h1>Gérer les enregistrements :</h1>
		...
		<?php 
			if(($Chercher) && $tabenr){
				echo "<h2 align='center'>Résultats</h2><table border='1' align='center'><tr><td>Nom</td><td>IP</td><td></td></tr>";
				foreach($tabenr as $key => $value){
					echo"<tr><td>".$value['nom']."</td><td>".$value['ip']."</td><td>
						<form method='POST'>
						<INPUT border=0 src='images/link_delete.png' type='image' Value='suppr_".$value['nom']."' name='suppr'> \n
						</td></tr>";
				}
				echo"</table>";
			}
