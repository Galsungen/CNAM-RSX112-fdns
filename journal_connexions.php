...
if(($_SESSION['nomusr'] == "") && (($_SESSION['role'] != 'admin') || ($_SESSION['role'] != 'ecrivain')))
{
	...
	$sql = "INSERT INTO `gd_journal` (`utilisateur`, `action`, `ip`, `nom`, `date`, `HHmmss`) VALUES ('', 'acces sans session au journal des connexions', '".mysql_real_escape_string($ip_visiteur)."', '', '".mysql_real_escape_string($date_jour)."', '".mysql_real_escape_string($heure_connexion)."');";
...
else
{
	...
	$sql = "INSERT INTO `gd_journal` (`utilisateur`, `action`, `ip`, `nom`, `date`, `HHmmss`) VALUES ('".mysql_real_escape_string($_SESSION['nomusr'])."', 'consultation du journal des connexions', '".mysql_real_escape_string($ip_visiteur)."', '', '".mysql_real_escape_string($date_jour)."', '".mysql_real_escape_string($heure_connexion)."');";

	$result = mysql_query($sql);
	if (!$result) {die('Requete invalide : ' . mysql_error());}
	
	//On récupère l'ensemble des enregistrements du journal
	$sql = "SELECT * FROM `gd_connexions`;";
...
//On affiche le résultat sous la forme d'un tableau à colonnes
	echo "<html><head><title>Journal des connexions</title></head><body><h1 align='center'>Journal des connexions</h1>";
	echo "<table border='1' align='center'>";
	echo "<tr><td>Date</td><td>Heure</td><td>Login</td><td>IP de l'utilisateur</td></tr>";
	foreach($tabjournal as $key => $value){
		echo "<tr><td>".$tabjournal[$key]['date']."</td><td>".$tabjournal[$key]['HHmmss']."</td><td>".$tabjournal[$key]['login']."</td><td>".$tabjournal[$key]['ip']."</td></tr>";
	}
	echo "</table>";
